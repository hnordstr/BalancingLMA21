"""This file was used to read solar data from the nordic countries (only Sweden)
...
# #
#### SWEDEN #####
# pv_info = pd.DataFrame(columns=['Kommun', 'PV', 'Longitude', 'Latitude', 'Area'])
...
# for a in se_areas:
#    df = pv_info.loc[pv_info['Area'] == a]
#    power_centres['Longitude'][a] = (df['Longitude'] * df['PV']).sum()/ df['PV'].sum()
#    power_centres['Latitude'][a] = (df['Latitude'] * df['PV']).sum() / df['PV'].sum()
#
#
# m = folium.Map([60,15], zoom_start=5, tiles='cartodbpositron')
# for _,f in bzs_in.iterrows():
...
#    folium.Marker(location=[power_centres['Latitude'][a], power_centres['Longitude'][a]]).add_to(m)
#
# for _,i in pv_info.iterrows():
#    # plot weighted point
#    if i['PV'] == 0.000:
#       pass
#    else:
#       folium.CircleMarker(location=[i['Latitude'], i['Longitude']], radius=2, weight=i['PV']/2).add_to(m)
#
#
# m.show_in_browser()
#
